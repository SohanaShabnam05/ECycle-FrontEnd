  ...
  useEffect(() => {
    if (clientLocation) {
        ...
        (event: { result: { geometry: any; place_name: any } }) => {
          ...
          if (geometry && geometry.coordinates) {
            ...
            const popup = new Popup().setHTML(
              `<h3 class="font-bold text-cyan-600 text-2xl">Selected Location</h3>
      ...
      sortedFacilities.forEach((facility, index) => {
        const popup = new Popup().setHTML(
          `<h3 class="font-bold text-cyan-600 text-2xl">${facility.name}</h3>
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row pt-24 md:pt-14 my-2 md:mt-8">
      {clientLocation ? (
        ...
        ${selectedFacility === index ? "bg-cyan-200" : ""}`}
                onClick={() => {
                  setSelectedFacility(index);
                }}
              >
        ...
        <div className="px-4 w-full md:h-[70vh] py-16 md:pt-64 md:py-24 md:container text-center">
          <div className="flex flex-col items-center justify-center px-10">
            ...
            <div className="text-black text-center text-xl md:text-3xl mt-4">
              ...
              <button onClick={handleAllowLocationClick} className="bg-cyan-500 text-white font-bold text-xl py-3 px-6 mt-8 rounded-full hover:bg-white-700">
                Allow Location
              </button>
          </div>
        </div>
